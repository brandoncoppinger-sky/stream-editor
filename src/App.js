        ...
        <DropdownButton id="dropdown-basic-button" title="Select Stream">
          {
            this.state.streamData.map(
              (data, index) => (<Dropdown.Item key={`${index}-button`} onClick={() => this.selectStream(data)}> {data.name}</Dropdown.Item>)
            )
          }
        </DropdownButton>
        </center>
        {/* This is a conditional - if selected stream exists render this component */}
        {this.state.selectedStream && <StreamDetails streamData={this.state.selectedStream} />}
      </div>
    );
  }
}
